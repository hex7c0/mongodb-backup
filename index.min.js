...
function writeMetadata(collection, metadata, next) {
    return collection.indexes(function(err, indexes) {
        return err ? (error(err), next(null)) : (fs.writeFileSync(metadata + collection.collectionName, JSON.stringify(indexes), {
            encoding: "utf8"
        }), next(null));
    });
}
...
function toJson(docs, collectionPath, next) {
    ...
    return 1 > last ? next(null) : docs.forEach(function(doc) {
        ...
        return 1 > last ? next(null) : collections.forEach(function(collection) {
            ...
            makeDir(name + collection.collectionName + "/", function(err, name) {
                return meta(collection, metadata, function() {
                    return collection.find(query).snapshot(!0).toArray(function(err, docs) {
                        return err ? last === ++index ? next(err) : error(err) : parser(docs, name, function(err) {
            ...
            makeDir(name + collection.collectionName + "/", function(err, name) {
                return meta(collection, metadata, function() {
                    return collection.parallelCollectionScan({
                        ...
                        if (0 === left) return last === ++index ? next(null) : null;
                        for (var i = 0; left > i; ++i) cursors[i].toArray(function(err, docs) {
                            return err ? last === ++index ? next(err) : error(err) : parser(docs, name, function(err) {
                                return err ? last === ++index ? next(err) : error(err) : last === ++index ? next(null) : null;
                            });
                        });
