...
function writeMetadata(collection, metadata, next) {
    collection.indexes(function(err, indexes) {
        return null !== err ? (error(err), next()) : (fs.writeFileSync(metadata + collection.collectionName, JSON.stringify(indexes), {
            encoding: "utf8"
        }), void next());
    });
}
...
function makeDir(path, next) {
    fs.stat(path, function(err, stats) {
        null !== err && "ENOENT" === err.code ? (logger("make dir at " + path), fs.mkdir(path, next(null, path))) : void 0 !== stats && stats.isDirectory() === !1 ? (logger("make dir at " + path), 
        ...
        if (fs.statSync(database).isDirectory() !== !1) {
            ...
            delete collections[collections.indexOf(".metadata")]), collections.forEach(function(second) {
                ...
                fs.statSync(collection).isDirectory() !== !1 && (fs.readdirSync(collection).forEach(function(third) {
                    ...
                    void 0 !== next && next(null, document), fs.unlinkSync(document);
            ...
            void makeDir(name + collection.collectionName + "/", function(err, name) {
                meta(collection, metadata, function() {
                    collection.find(query).toArray(function(err, docs) {
                        return null !== err ? last === ++index ? next(err) : error(err) : void parser(docs, name, function(err) {
...
function someCollections(db, name, query, metadata, parser, next, collections) {
    ...
    return 1 > last ? next(null) : void collections.forEach(function(collection) {
        db.collection(collection, function(err, collection) {
            return logger("select collection " + collection.collectionName), null !== err ? last === ++index ? next(err) : error(err) : void makeDir(name + collection.collectionName + "/", function(err, name) {
                meta(collection, metadata, function() {
                    collection.find(query).toArray(function(err, docs) {
                        return null !== err ? last === ++index ? next(err) : error(err) : void parser(docs, name, function(err) {
                            return null !== err ? last === ++index ? next(err) : error(err) : last === ++index ? next(null) : null;
                        });
                    });
                });
            });
...
function wrapper(my) {
    ...
    if ("function" == typeof my.parser) parser = my.parser; else switch (my.parser) {
      case "bson":
        BSON = require("bson").BSONPure.BSON, parser = toBson;
        break;
    ...
    meta = my.metadata === !0 ? writeMetadata : function(a, b, c) {
        ...
        if (logger("db open"), null !== err) return error(err);
            ...
            makeDir(name + db.databaseName + "/", function(err, name) {
                var go = function() {
                    discriminator(db, name, my.query, metadata, parser, function() {
                        return logger("db close"), db.close(), null !== my.tar ? makeDir(my.root, function(err, name) {
                            logger("make tar file at " + name + my.tar);
                            var dest = fs.createWriteStream(name + my.tar), packer = require("tar").Pack().on("error", error).on("end", function() {
                                rmDir(root), callback();
                            });
                ...
                my.metadata === !1 ? go() : (metadata = name + ".metadata/", makeDir(metadata, function() {
                    go();
                }));
            });
        });
...
var fs = require("fs"), client = require("mongodb").MongoClient, BSON, logger, meta;
