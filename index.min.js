...
function makeDir(path, next) {
    fs.stat(path, function(err, stats) {
        return err && "ENOENT" === err.code ? (logger("make dir at " + path), fs.mkdir(path, function(err) {
            next(err, path);
        })) : stats && stats.isDirectory() === !1 ? (logger("unlink file at " + path), fs.unlink(path, function(err) {
            return err ? next(err) : (logger("make dir at " + path), void fs.mkdir(path, function(err) {
                next(err, path);
            }));
        })) : void next(null, path);
    });
...
function wrapper(my) {
    ...
    meta = my.metadata === !0 ? writeMetadata : function(a, b, c) {
        ...
        makeDir(root, function(err, name) {
            return err ? callback(err) : void makeDir(name + db.databaseName + "/", function(err, name) {
                function go() {
                ...
                return err ? callback(err) : void (my.metadata === !1 ? go() : (metadata = name + ".metadata/", 
                makeDir(metadata, go)));
            });
        });
    });
